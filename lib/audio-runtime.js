...
export class AudioRuntime {
  constructor(ctx, resolver, trackKeys = ["kick", "snare", "hihat", "openhat"], options = {}) {
    ...
    this.options = {
      ...
      lookahead: options.lookahead || 0.1,
      tickInterval: options.tickInterval || 0.025,
    ...
    this.stepCount = this.options.stepCount;
  ...
  scheduler() {
    ...
    while (this.nextNoteTime < this.ctx.currentTime + this.options.lookahead) {
      ...
      if (this.options.onStep) {
        this.options.onStep(this.nextStepIndex, stepTime);
      }

      this.nextStepIndex = (this.nextStepIndex + 1) % this.stepCount;
      this.nextNoteTime += secondsPerStep;

      if (this.nextStepIndex === 0 && this.options.onLoop) {
        this.options.onLoop(this.nextNoteTime);
      }
    }
  ...
  playStep(stepIndex, time) {
    ...
    if (stepEvent.harmony === null) {
      this.releaseHarmony(time);
    } else if (stepEvent.harmony) {
      this.playHarmony(stepEvent.harmony, time);
    }
    if (stepEvent.drums && stepEvent.drums.length > 0) {
      const stepDuration = 60 / this.bpm / 4;
      stepEvent.drums.forEach((drum) => {
        const drumTime = time + (drum.offset || 0) * stepDuration;
        this.playDrum(drum.trackKey, drumTime, drum.velocity);
      });
    }
    if (stepEvent.bass && stepEvent.bass.length > 0) {
      stepEvent.bass.forEach((bass) => {
        this.playBassNote(bass.note, time, bass.tick, bass.length);
      });
    }
  }
  ...
  playRhythm(chord, time) {
    ...
    const strumLength = this.strumLength || 0.12;

    if (player && preset) {
      player.queueChord(this.ctx, this.graph.rhythmGain, preset, time, parsed.midi, strumLength, 0.8);
      return;
    }
    ...
    playInternalChord(this.ctx, parsed.frequencies, output, time, strumLength, 0.15, params);
  }

  playHarmony(chord, time) {
    if (chord === "_") return;
    const parsed = this.parseChord(chord, 60);
    if (!parsed) {
      this.releaseHarmony(time);
      return;
    }
  ...
  playBassNote(midi, time, tick, length = 1) {
    const velocity = 1;
    const key = `bass_${tick}`;
    this.releaseBassNote(key, time);

    const tickDuration = 60 / this.bpm / 4 / 4;
    const duration = Math.max(tickDuration * 0.8, tickDuration * length * 0.88);
    ...
    const voice = createBassVoice(
      ...
      velocity,
      releaseAt,
      this.bassParams.harmonics || 0,
      this.bassParams.harmonicShape || "square",
      this.bassParams.harmonicFilter || 300
    );
