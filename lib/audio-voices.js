...
export function createBassVoice(audioContext, midi, bassGainNode, time, layers, glide = 0.04, filter = 420, release = 0.22, glideFromMidi = null, velocity = 1, releaseAt = null) {
  ...
  const voices = layers.map((layer) => {
    ...
    osc.start(time);
    if (releaseAt !== null) {
      envelope.gain.setTargetAtTime(0.0001, releaseAt, release);
      osc.stop(releaseAt + release + 0.1);
    }
    return { osc, envelope };
  });
