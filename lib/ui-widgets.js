...
export function parseDrumPattern(value) {
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    if (ch === "[") {
      const end = raw.indexOf("]", i + 1);
      if (end < 0) break;
      chars.push(...raw.slice(i + 1, end).split(""));
      i = end;
      continue;
    }
    if (/[\s|]/.test(ch)) continue;
    chars.push(ch === "." || ch === "0" ? "-" : ch);
  }
  const maxSteps = 32;
  if (!chars.length || chars.length > maxSteps || maxSteps % chars.length !== 0) return null;
  const expanded = [];
  while (expanded.length < maxSteps) expanded.push(...chars);
